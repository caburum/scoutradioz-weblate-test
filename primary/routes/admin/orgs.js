...
const logger = require('log4js').getLogger('orgs');
...
router.post('/create', wrap(async (req, res) => {
	...
	const nickname = req.body.nickname;
	const teamKeyOrKeys = req.body.team_key;
...
router.post('/login-to-org', wrap(async (req, res) => {
	...
	const user = await utilities.findOne('users', {_id: req.user._id});
	const userRole = await utilities.findOne('roles', {role_key: user.role_key});
	...
	if (comparison === true && user.password !== 'disabled' && userRole.access_level >= process.env.ACCESS_GLOBAL_ADMIN) {
